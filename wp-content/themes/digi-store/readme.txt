== Digi Store ==

Contributors: Code Themes
Version: 1.1.2
License: GPLv3 or later
License URI: http://www.gnu.org/licenses/gpl-3.0.html
Requires at least: 4.0
Tested up to: 4.9.2

Tags: one-column, two-columns, threaded-comments, flexible-header, right-sidebar, custom-background, custom-header, custom-menu, editor-style, featured-images, sticky-post, theme-options, translation-ready, post-formats, custom-logo, footer-widgets, e-commerce, blog

== Description ==

Digi Store is an eCommerce WordPress theme which takes your digital store in next level. This theme suitable for promoting and advertising your products and building a robust digital or online store for your business. It is based on BootStrap CSS Framework which gives an alluring visual experience.More importantly, Digi Store is powered by the world’s most prominent and favored eCommerce plugin i.e WooCommerce. You can easily handle your customer and order. It is a fully responsive theme and works precisely well in any screen size and resolution. It is very user-friendly and gives you an excellent experience with its effortless customization options.

== Installation ==

1. In your admin panel, go to Appearance -> Themes and click the 'Add New' button.
2. Click Upload and Choose File, then select the theme's .zip file. Click Install Now.
3. Click on the 'Activate' button to use your new theme right away.
4. Go to http:// for a guide on how to customize this theme.
5. Navigate to Appearance > Customize in your admin panel and customize to taste.

== Required Plugins ==

*  WooCommerce

== Recommended Plugins ==
*  Jetpack
*  Contact Form 7


== Copyright ==

Digi Store WordPress Theme, Copyright 2017, Code Themes
Digi Store is distributed under the terms of the GNU GPL

Digi Store WordPress Theme is derived from Edge WordPress Theme, Copyright ThemeFreesia Team
Edge WordPress Theme is distributed under the terms of the GNU GPL

This program is free software: you can redistribute it and/or modify
it under the terms of the GNU General Public License as published by
the Free Software Foundation, either version 3 of the License, or
(at your option) any later version.

This program is distributed in the hope that it will be useful,
but WITHOUT ANY WARRANTY; without even the implied warranty of
MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the
GNU General Public License for more details.

Digi Store Theme bundles the following third-party resources:

Based on Underscores http://underscores.me/,
(C) 2012-2016 Automattic, Inc.,
[GPLv2 or later](https://www.gnu.org/licenses/gpl-2.0.html)

Front-end framework based on Materialize
Licenses: MIT. https://github.com/Dogfalo/materialize/blob/master/LICENSE
Source: http://materializecss.com/

== Images ==
	https://pixabay.com/en/girl-music-pink-fashion-listen-1990347/

	Image name: wparmy.png (Assets of codethemes.co)


== Fonts ==

	Poppins
	License: Open Font License
	Source: https://fonts.google.com/specimen/Poppins?selection.family=Poppins

== Scripts ==

	sticky.js
	Licenses: https://github.com/garand/sticky/blob/master/LICENSE.md
	Source: http://stickyjs.com/

	HTML5.js
	Licenses: HTML5-Shiv is Licensed under the MIT and GPL licenses
	Source: https://github.com/afarkas/html5shiv

	jQuery.cycle.all
	Licenses: JQuery Cylcle all is Licensed under the MIT and GPL licenses
	Source: http://jquery.malsup.com/cycle2/

	color scheme control:
	customize-preview:
	Licenses: GNU General Public License v3 or later
	Source: https://wordpress.org/themes/twentyfifteen
	Copyright: Automattic, automattic.com

== Styles ==

	Bootstrap v3.3.7 (http://getbootstrap.com)
	Copyright 2011-2016 Twitter, Inc.
	Licensed under MIT (https://github.com/twbs/bootstrap/blob/master/LICENSE)

	Slick Carousel v1.6.0, Copyright (c) 2014 Ken Wheeler.
	License: MIT
	License URI: https://opensource.org/licenses/MIT
	Source: http://kenwheeler.github.io/slick/

	 Font Awesome 4.6.3 by @davegandy - http://fontawesome.io - @fontawesome
	 License - http://fontawesome.io/license (Font: SIL OFL 1.1, CSS: MIT License)

== Change Log ==

= 1.1.2 =
* 6 February 2018
* Prefix added to function
* Minor issues fixed

= 1.1.1 =
* 30 January 2018
* Issues with themecheck fixed
* Prefix is added in localize object
* Other issues fixes.

= 1.1.0 =
* 16 January 2018
* Coding issues fixed.
* Design issues fixed.
* Responsive issues fixed.
* Breadcrumb issues fixed.
* Unwanted template removed.

= 1.0.0 =
* 27 October 2017
* Initial release
